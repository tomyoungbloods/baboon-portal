...
export default {
    ...
    computed: {
    openTasks() {
      return this.tasks.filter(tasks => tasks.status === "open" && tasks.company_id === this.id);
    },
    busyTasks() {
      return this.tasks.filter(tasks => tasks.status === "in behandeling" && tasks.company_id === this.id);
    },
    closedTasks() {
